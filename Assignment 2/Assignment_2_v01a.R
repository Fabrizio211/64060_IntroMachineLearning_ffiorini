...
ggplot(train_df, aes(x=Income, y=CCAvg, color=train_df$Personal Loan)) + geom_point()
...
#TRAINING / K-NN MODEL
#remember to move Y value to the first place!
#predictors
...
set.seed(1234)
predicted_labels_prob <- knn(train_predictors, 
                            valid_predictors, 
                            cl=train_labels, k=1, prob=TRUE )
class_prob <- attr(predicted_labels_prob, 'prob')
head(class_prob)
...
new_cust <- c(40, 10, 84, 2, 2, 0, 0, 0, 1, 1, 0, 1, 0)#controllare
...
predicted_labels_prob2 <-knn(train_predictors,
                  m,
                  cl=train_labels,
                  k=1, prob = TRUE)

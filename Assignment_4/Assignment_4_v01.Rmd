...
# In this assignment I am going to apply a type of cluster analysis, n kmeans 
...
For the first part of the assignment I will focus on a subset of the original dataset that consists only of numerical variable.
...
From the charts plotter, we can see that the elbow in the line is when k=2, while the Silhouette Method gives us k=5 as a result.
...
We can also retrive the size of each cluster. Cluster n.1 is the most large, while cluster n.3 has only 2 firms.
...
For the sake of a better interpretation of the cluster analysis, I decided to run again the model using only 3 clusters.
In this way, it is easier to identify and manage the cluster in the analysis. From the second chart we see that cluster n.1 is formed by companies with a low Net Profit Margin and a high Price/Earnings ratio, while firms in cluster n.3 have low Asset Turnover and Return on Asset (ROA) but high Leverage and Estimated Revenue Growth.
...
pharma.cat <-  pharma %>% select(c(11,12,13)) %>% 
    mutate(Cluster = km.pharma$cluster) %>% arrange(desc(Cluster))
```
